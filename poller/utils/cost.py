...
def estimate_ec2_cost(instance_type: str, launch_time: datetime) -> Decimal:
    ...
    if rate is None:
        logger.warning(f"No hourly rate for EC2 type '{instance_type}' — cost set to 0")
        return Decimal("0")
...
def estimate_rds_cost(db_class: str, create_time: datetime) -> Decimal:
    ...
    if rate is None:
        logger.warning(f"No hourly rate for RDS class '{db_class}' — cost set to 0")
        return Decimal("0")
...
def estimate_ebs_volume_cost(
    ...
    if rate is None:
        logger.warning(f"No monthly rate for EBS type '{volume_type}' — cost set to 0")
        return Decimal("0")
