...
Run: python3 -c "import app.routes.test_query_behavior as t; t.run_tests()"
"""
...
def test_alert_filters():
    ...
    test_cases = [
        {
            "name": "No filters",
            "filters": {"severity": None, "alert_type": None, "status": "active"},
            "expected_where_equivalent": "a.resolved_at IS NULL",
        },
        {
            "name": "Severity filter only",
            "filters": {"severity": "critical", "alert_type": None, "status": "active"},
            "expected_where_equivalent": (
                "a.severity = 'critical' AND a.resolved_at IS NULL"
            ),
        },
        {
            "name": "All filters",
            ...
            "expected_where_equivalent": (
                "a.severity = 'warning' AND a.alert_type = 'ec2_long_running' "
                "AND a.resolved_at IS NOT NULL"
            ),
        },
        {
            "name": "Status = all (edge case)",
            "filters": {"severity": None, "alert_type": None, "status": "all"},
...
def test_resource_filters():
    ...
    test_cases = [
        {
            "name": "No filters",
            "filters": {"type": None, "state": None, "region": None},
            "expected_where_equivalent": "is_active = TRUE",
        },
        {
            "name": "Type filter only",
            "filters": {"type": "ec2", "state": None, "region": None},
            "expected_where_equivalent": "is_active = TRUE AND resource_type = 'ec2'",
        },
        {
            "name": "All filters",
            "filters": {"type": "ec2", "state": "running", "region": "ap-south-1"},
            "expected_where_equivalent": (
                "is_active = TRUE AND resource_type = 'ec2' AND state = 'running' "
